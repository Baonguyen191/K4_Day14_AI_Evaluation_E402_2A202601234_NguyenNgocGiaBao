    ...
    def evaluate_context_precision(
        ...
        for k, chunk in enumerate(contexts, start=1):
            ...
            if not expected_tokens:
                ...
                is_relevant = overlap_ratio >= relevance_threshold
            if is_relevant:
                num_relevant += 1
        ...
        if num_relevant == 0:
            return 0.0
        return min(1.0, max(0.0, precision_sum / num_relevant))
...
def rerank_by_overlap(contexts: list[str], query: str) -> list[str]:
    ...
    # Python's sort is stable, so ties preserve the retriever's original order.
    return sorted(
        contexts,
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            f"You are an impartial evaluator. Score the AI answer below on each "
            f"criterion using a float between 0.0 and 1.0. "
            f"Return ONLY a JSON object mapping criterion name to score.\n\n"
            f"Question: {question}\n\n"
            f"Answer: {answer}\n\n"
            f"Rubric:\n{rubric_lines}\n\n"
            f"JSON scores:"
        ...
        try:
            ...
            # Fall back for any missing criterion
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Collect all individual scores across the batch
        ...
        # Positional bias: first batch entry scores consistently higher than rest
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_context_precision": _avg(precision_vals) if precision_vals else None,
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            # Keep cell content from breaking the Markdown table.
            cells = [
                f"F{index:03d}",
                failure_type,
                root_cause,
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_by_type = {
            "hallucination": "Add claim-level grounding checks and reject unsupported facts before returning the answer",
            "irrelevant": "Clarify intent detection and add question-focused prompt examples",
            "incomplete": "Increase retrieval coverage or context window and add complete-answer examples",
            "off_topic": "Add an intent guardrail and regression cases for unrelated questions",
            "refusal": "Relax overly broad refusal rules while preserving safety and privacy checks",
            "unknown": "Inspect the failing trace and add a targeted regression test",
        }
        for failure_type in categories:
            suggestion = suggestion_by_type.get(failure_type)
            if suggestion:
                suggestions.append(suggestion)

        # Keep the output useful even when all failures share one category.
        fallbacks = [
            "Review retrieved evidence and improve chunking or ranking for missing facts",
            "Add representative golden examples and run them in CI as a quality gate",
        ]
